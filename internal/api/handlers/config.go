...
func writeConfigToFile(cfg *config.Config) error {
	...
	// Debug: Log first 500 chars of marshaled YAML to verify admin section
	yamlPreview := string(data)
	if len(yamlPreview) > 500 {
		yamlPreview = yamlPreview[:500]
	}
	log.Debug().Str("yaml_preview", yamlPreview).Msg("Marshaled YAML preview")
